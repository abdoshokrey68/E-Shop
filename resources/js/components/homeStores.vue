        ...
        <div class="row">
            <div class="col-md-3 blog" id="blog">
                <div class="sidebar p-2">
                    <h3 class="sidebar-title"> <i class="fas fa-search"></i> {{lang.search}} </h3>
                    <div class="sidebar-item search-form">
                        <input list="search" class="form-control dynamic" name="search" v-model="search" id="search" placeholder="Search">
                        <datalist id="search">
                            <option v-for="store in stores" :key="store.index" value="">
                                {{store.name}}
                            </option>
                        </datalist>
                    ...
                    <div class="sidebar-item categories p-2 m-0">
                        <h3 class="sidebar-title"> <i class="fas fa-truck mr-2 ml-2"></i> {{lang.delivery}} </h3>
                        <ul>
                            <li class="m-0">
                                <input type="checkbox" v-model="store_delivery" value="1" name="store_delivery" id="deli_available">
                                <label for="deli_available"> {{lang.available}} </label>
                            </li>
                            <li class="m-0">
                                <input type="checkbox" v-model="store_delivery" value="0" name="store_delivery" id="deli_notavailable">
                                <label for="deli_notavailable"> {{lang.not_available}} </label>
                            </li>
                        </ul>
                    </div> <!-- End Delivary -->
                    ...
                    <div class="sidebar-item categories p-2 m-0">
                        <h3 class="sidebar-title"> <i class="fas fa-money-check mr-2 ml-2"></i> {{lang.payment}} </h3>
                        <ul>
                            <li>
                                <input type="checkbox" v-model="store_payment" value="1" name="store_payment" id="pay_available">
                                <label for="pay_available"> {{lang.available}} </label>
                            </li>
                            <li>
                                <input type="checkbox" v-model="store_payment" value="0" name="store_payment" id="pay_notavailable">
                                <label for="pay_notavailable"> {{lang.not_available}} </label>
                            </li>
                        </ul>
                    </div> <!-- End Delivary -->
            ...
            <div class="col-md-9 stores-box">
                <h1 class="col-md-12 text-center text-style"> <div class="fas fa-store"></div> {{lang.markets}} </h1>
                <h5 v-if="search" class="mb-2 h4"> {{search}} </h5>
                <hr>
                <br>
                <div class="row" id="all-stores">
                    <div v-for="store in stores" :key="store.index" v-if="store_delivery.includes(String(store.delivery)) && store_payment.includes(String(store.payment))" class="col-lg-3 col-md-6 mt-2 my-store">
                        <div class="icon-box store-box col-md-12 p-0">
                            <div class="">
                                <!-- <a :href="'/store/'+store.id" target="_blank"> -->
                                    <img v-if="store.image" :src="'/img/stores/'+store.image" class="home-stores-image" alt="store image">
                                    <img v-else src="/img/stores/default.png" class="home-stores-image" alt="store image">
                                <!-- </a> -->
                            </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            searchstores()
            {
                axios.get(`/api/searchstores?search=${this.search}`)
                .then(res => {
